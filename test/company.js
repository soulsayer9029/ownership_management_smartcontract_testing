...
contract("Company", (accounts) => {
    ...
    it("6. master should be able to remove owners", async () => {
      const company = await Company.deployed();
      const owners=await company.getOwners()
      const t=await company.addOwner(user2)
      const tx = await company.removeOwner(user2);
      console.log((await company.getOwners()))
      truffleAssert.eventEmitted(tx, "OwnerRemoval", (ev) => {
        return ev.owner == user2;
      });
      assert.deepEqual(await company.getOwners(), owners);
    });
